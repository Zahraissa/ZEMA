...
import { ArrowLeft, Upload, X, FileText, Save, FolderTree, FileEdit, FileText as FileTextIcon, ToggleLeft, ToggleRight, Sparkles, CheckCircle2 } from 'lucide-react';
...
import { enhancedGuidelinesService, GuidelinesGroup } from '@/services/enhancedGuidelinesService';
import Swal from 'sweetalert2';

const AddViwango: React.FC = () => {
  ...
  const loadGroups = async () => {
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Makosa',
        text: 'Imeshindwa kupakia makundi. Tafadhali jaribu tena.',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (groups.length === 0) {
      await Swal.fire({
        icon: 'error',
        title: 'Hakuna Makundi',
        text: 'Tafadhali ongeza kikundi cha miongozo kwanza.',
        confirmButtonColor: '#3085d6'
      });
      return;
    }
    ...
    try {
      ...
      // Create standard data with defaults
      const standardData = {
        title: documentName.trim(),
        description: description.trim() || '', // Description from form
        group_id: selectedGroupId!, // Use selected group
        standard_type: 'Guideline', // Default type
        maturity_level: 'established', // Default maturity level
        version: '1.0', // Default version
        status: (isActive ? 'active' : 'inactive') as 'active' | 'inactive' | 'draft',
        date_published: new Date().toISOString().split('T')[0], // Today's date
        featured: false,
        order: 1,
        file: selectedFile!
      };
      ...
      // Call API
      const response = await enhancedGuidelinesService.createStandard(standardData);
      ...
      console.error('Error creating viwango:', error);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Ongeza viwango mpya kwenye mfumo</p>
              </div>
            </div>
