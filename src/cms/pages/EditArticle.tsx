...
import { ArrowLeft, Save, Eye, Upload, Calendar, User, BookOpen } from "lucide-react"
...
export default function EditArticle() {
  ...
  const fetchArticle = async (articleId: number) => {
    try {
      ...
      const response = await managementAPI.getNews({ id: articleId })
      const article = response.data.data[0] || response.data.data
      ...
      if (article) {
        setFormData({
          ...
          publishDate: article.publish_date ? new Date(article.publish_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          status: article.status || "draft"
        })
        
        if (article.image) {
          setCurrentImage(`http://102.214.44.122:8080/storage/${article.image}`)
          setImagePreview(`http://102.214.44.122:8080/storage/${article.image}`)
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await managementAPI.updateNews(parseInt(id!), formDataToSend)
  ...
  const removeImage = () => {
    setImageFile(null)
    if (imagePreview && imagePreview !== currentImage) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-normal tracking-tight">Edit Article</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Update the image for your article
              </CardDescription>
            </CardHeader>
            <CardContent>
              {imagePreview ? (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-green-600 text-center">
                    {imageFile ? `New image selected: ${imageFile.name}` : 'Current article image'}
                  </p>
                </div>
              ) : (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              >
                ...
                {loading ? 'Updating Article...' : (formData.status === 'published' ? 'Update & Publish' : 'Update Article')}
              </Button>
