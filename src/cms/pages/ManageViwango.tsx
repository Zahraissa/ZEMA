...
const ManageViwango: React.FC = () => {
  const [viwango, setViwango] = useState<GuidelineStandard[]>([]);
  const [filteredViwango, setFilteredViwango] = useState<GuidelineStandard[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [isLoading, setIsLoading] = useState(true);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [viwangoToDelete, setViwangoToDelete] = useState<GuidelineStandard | null>(null);
  ...
  useEffect(() => {
    filterViwango();
  }, [searchQuery, selectedStatus, viwango]);

  const loadViwango = async () => {
    try {
      setIsLoading(true);
      
      const standards = await enhancedGuidelinesService.getAllGuidelinesStandards();
      console.log('Viwango response:', standards);
      
      setViwango(standards);
      setFilteredViwango(standards);
  ...
  const filterViwango = () => {
    ...
    setFilteredViwango(filtered);
  };

  const handleDelete = async () => {
    if (!viwangoToDelete) return;

    try {
      await enhancedGuidelinesService.deleteStandard(viwangoToDelete.id);
      
      Swal.fire({
        icon: 'success',
        title: 'Imefanikiwa!',
        text: 'Viwango imefutwa kwa mafanikio.',
      ...
      console.error('Failed to delete viwango:', error);
      
      Swal.fire({
        icon: 'error',
        title: 'Makosa',
        text: 'Imeshindwa kufuta viwango. Tafadhali jaribu tena.',
        confirmButtonColor: '#ef4444'
      });
    } finally {
      setDeleteDialogOpen(false);
      setViwangoToDelete(null);
    }
  };

  const handleDownload = async (standard: GuidelineStandard) => {
    try {
      if (!standard.file_name || !standard.file_path) {
        Swal.fire({
          icon: 'warning',
          title: 'Faili Haipatikani',
          text: 'Faili ya viwango haipatikani kwa kupakua.',
          confirmButtonColor: '#3085d6'
        });
        return;
      }
      ...
      const fileUrl = `${STORAGE_BASE_URL}${standard.file_path}`;
      const a = document.createElement('a');
      a.href = fileUrl;
      a.download = standard.file_name;
      a.target = '_blank';
      ...
      Swal.fire({
        icon: 'success',
        title: 'Imepakuliwa!',
        text: `"${standard.file_name}" inapakuliwa`,
        timer: 2000,
        showConfirmButton: false
      });
    } catch (error) {
      console.error('Error downloading viwango:', error);
      Swal.fire({
        icon: 'error',
  ...
  const handleView = (standard: GuidelineStandard) => {
    if (!standard.file_path || !standard.file_name) {
      Swal.fire({
        icon: 'warning',
        title: 'Faili Haipatikani',
        text: 'Faili ya viwango haipatikani kwa kutazama.',
        confirmButtonColor: '#3085d6'
      });
      return;
    }
  ...
  const handleToggleStatus = async (standard: GuidelineStandard) => {
    try {
      const newStatus = standard.status === 'active' ? 'inactive' : 'active';
      
      // Fetch full standard data first
      const fullStandard = await enhancedGuidelinesService.getStandardById(standard.id);
      
      // Update standard status with all required fields
      await enhancedGuidelinesService.updateStandard({
        ...fullStandard,
        id: fullStandard.id,
        status: newStatus as 'active' | 'inactive' | 'draft',
        group_id: fullStandard.group_id,
      ...
      Swal.fire({
        icon: 'success',
        title: 'Imefanikiwa!',
        text: `Hali ya viwango imebadilishwa kuwa "${newStatus === 'active' ? 'Aktifu' : 'Haijaamilishwa'}"`,
        confirmButtonColor: '#10b981',
        timer: 2000,
        timerProgressBar: true
      });
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Usimamizi wa Viwango</h1>
                <p className="text-gray-600 mt-1">Dhibiti viwango zote za e-Government Authority</p>
              </div>
            </div>
            <Button 
              onClick={() => navigate('/cms/add-viwango')}
          ...
          <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Jumla ya Viwango</p>
                  <p className="text-3xl font-bold text-gray-900">{viwango.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Viwango za Aktifu</p>
                  <p className="text-3xl font-bold text-gray-900">
                    {viwango.filter(v => v.status === 'active').length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {viwango.filter(v => v.status === 'inactive').length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{filteredViwango.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader className="bg-gradient-to-r from-purple-50 to-purple-50/50 border-b border-gray-100">
            <CardTitle className="flex items-center gap-3 text-xl font-bold text-gray-900">
              ...
              <span>Viwango ({filteredViwango.length})</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {isLoading ? (
              <div className="flex items-center justify-center py-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 font-medium">Inapakia viwango...</p>
                </div>
              </div>
            ) : filteredViwango.length > 0 ? (
                        ...
                        <TableCell className="py-4 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem 
                                onClick={() => navigate(`/cms/edit-viwango/${item.id}`)}
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  {searchQuery || selectedStatus !== 'all' 
                    ? 'Hakuna viwango zinazofanana na vichujio ulivyochagua. Jaribu kubadilisha vichujio vyako.'
                    : 'Bado hujaongeza viwango yoyote. Bofya "Ongeza Viwango" kuanza kuongeza viwango mpya.'}
                </p>
                {(!searchQuery && selectedStatus === 'all') && (
                  <Button 
                    onClick={() => navigate('/cms/add-viwango')}
          ...
          <AlertDialogContent className="border-2 border-gray-200">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-base">
                Kitendo hiki hakiwezi kurekebishwa. Hii itafuta viwango <span className="font-semibold text-gray-900">"{viwangoToDelete?.title}"</span> 
