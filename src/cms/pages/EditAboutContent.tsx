...
export default function EditAboutContent() {
  ...
  const fetchAboutContent = async () => {
    try {
      setLoading(true)
      const response = await managementAPI.getAboutContent({ id })
      const content = response.data.data.find((item: AboutContent) => item.id === parseInt(id!))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await managementAPI.updateAboutContent(parseInt(id!), formDataToSend)
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true)
      await managementAPI.deleteAboutContent(parseInt(id!))
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-normal tracking-tight">Edit About Content</h1>
            <p className="text-muted-foreground">
              Update content for the about page
            </p>
          </div>
        </div>
        <Button
