...
const EditOrodha: React.FC = () => {
  ...
  const [orodha, setOrodha] = useState<Guide | null>(null);
  ...
  const loadOrodha = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        throw new Error('Failed to load orodha data');
      }
    } catch (error: any) {
  ...
  const removeFile = () => {
    ...
    if (orodha?.file_name && orodha?.file_path) {
      setExistingFile({
        name: orodha.file_name,
        path: orodha.file_path
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!orodha) {
      await Swal.fire({
        icon: 'error',
        title: 'Makosa',
        text: 'Taarifa za orodha hazipatikani.',
        confirmButtonColor: '#3085d6'
      });
      return;
    }
    ...
    try {
      ...
      formDataToSend.append('description', orodha.description || ''); // Keep existing description
      formDataToSend.append('category', 'Orodha ya Viwango na Miongozo'); // Keep category
      formDataToSend.append('status', isActive ? 'active' : 'inactive');
      formDataToSend.append('order', orodha.order?.toString() || '1');
      formDataToSend.append('featured', orodha.featured ? '1' : '0');
      ...
      if (response.success) {
        ...
        // Navigate back to manage orodha
        navigate('/cms/manage-orodha');
      } else {
        throw new Error(response.message || 'Failed to update orodha');
      }
      
    } catch (error: any) {
      console.error('Error updating orodha:', error);
      ...
      if (error.response) {
        if (error.response.data && error.response.data.message) {
          ...
          errorMessage = 'Orodha haijulikani.';
          ...
          <Card className="border-2 border-gray-100 shadow-lg">
            <CardContent className="flex items-center justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-gray-600 font-medium">Inapakia taarifa za orodha...</p>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Hariri orodha iliyopo kwenye mfumo</p>
              </div>
            </div>
