...
const ManageSampuli: React.FC = () => {
  const [sampuli, setSampuli] = useState<Guide[]>([]);
  const [filteredSampuli, setFilteredSampuli] = useState<Guide[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [isLoading, setIsLoading] = useState(true);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [sampuliToDelete, setSampuliToDelete] = useState<Guide | null>(null);
  ...
  useEffect(() => {
    filterSampuli();
  }, [searchQuery, selectedStatus, sampuli]);

  const loadSampuli = async () => {
    try {
      setIsLoading(true);
      
      // Fetch guides filtered by "Sample Templates" category
      const params: any = {
  ...
  const filterSampuli = () => {
    ...
    setFilteredSampuli(filtered);
  };

  const handleDelete = async () => {
    if (!sampuliToDelete) return;

    try {
      await managementAPI.deleteGuide(sampuliToDelete.id);
      
      Swal.fire({
        icon: 'success',
        title: 'Imefanikiwa!',
        text: 'Sampuli imefutwa kwa mafanikio.',
      ...
      console.error('Failed to delete sampuli:', error);
      
      Swal.fire({
        icon: 'error',
        title: 'Makosa',
        text: 'Imeshindwa kufuta sampuli. Tafadhali jaribu tena.',
        confirmButtonColor: '#ef4444'
      });
    } finally {
      setDeleteDialogOpen(false);
      setSampuliToDelete(null);
    }
  };
    ...
    try {
      if (!guide.file_name || !guide.file_path) {
        Swal.fire({
          ...
          text: 'Faili ya sampuli haipatikani kwa kupakua.',
          confirmButtonColor: '#3085d6'
        });
        return;
      }
      ...
      console.error('Error downloading sampuli:', error);
      Swal.fire({
        icon: 'error',
  ...
  const handleView = (guide: Guide) => {
    if (!guide.file_path || !guide.file_name) {
      Swal.fire({
        icon: 'warning',
        title: 'Faili Haipatikani',
        text: 'Faili ya sampuli haipatikani kwa kutazama.',
        confirmButtonColor: '#3085d6'
      });
      return;
    }
  ...
  const handleToggleStatus = async (guide: Guide) => {
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Imefanikiwa!',
        text: `Hali ya sampuli imebadilishwa kuwa "${newStatus === 'active' ? 'Aktifu' : 'Haijaamilishwa'}"`,
        confirmButtonColor: '#10b981',
        timer: 2000,
        timerProgressBar: true
      });
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Usimamizi wa Sampuli</h1>
                <p className="text-gray-600 mt-1">Dhibiti sampuli zote za e-Government Authority</p>
              </div>
            </div>
            <Button 
              onClick={() => navigate('/cms/add-sampuli')}
          ...
          <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Jumla ya Sampuli</p>
                  <p className="text-3xl font-bold text-gray-900">{sampuli.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Sampuli za Aktifu</p>
                  <p className="text-3xl font-bold text-gray-900">
                    {sampuli.filter(s => s.status === 'active').length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {sampuli.filter(s => s.status === 'inactive').length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{filteredSampuli.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-2 border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader className="bg-gradient-to-r from-purple-50 to-purple-50/50 border-b border-gray-100">
            <CardTitle className="flex items-center gap-3 text-xl font-bold text-gray-900">
              ...
              <span>Sampuli ({filteredSampuli.length})</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {isLoading ? (
              <div className="flex items-center justify-center py-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 font-medium">Inapakia sampuli...</p>
                </div>
              </div>
            ) : filteredSampuli.length > 0 ? (
                        ...
                        <TableCell className="py-4 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuSeparator />
                              <DropdownMenuItem 
                                onClick={() => navigate(`/cms/edit-sampuli/${item.id}`)}
              ...
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">
                  Hakuna sampuli zinazopatikana
                </h3>
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  {searchQuery || selectedStatus !== 'all' 
                    ? 'Hakuna sampuli zinazofanana na vichujio ulivyochagua. Jaribu kubadilisha vichujio vyako.'
                    : 'Bado hujaongeza sampuli yoyote. Bofya "Ongeza Sampuli" kuanza kuongeza sampuli mpya.'}
                </p>
                {(!searchQuery && selectedStatus === 'all') && (
                  <Button 
                    onClick={() => navigate('/cms/add-sampuli')}
          ...
          <AlertDialogContent className="border-2 border-gray-200">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-base">
                Kitendo hiki hakiwezi kurekebishwa. Hii itafuta sampuli <span className="font-semibold text-gray-900">"{sampuliToDelete?.title}"</span> 
