...
import { Search, FileText, Eye, Download, ChevronDown, Calendar, Tag, Star, TrendingUp, Sparkles, FileCheck, ArrowRight, List, Award, CheckCircle } from 'lucide-react';
...
const OrodhaViwangoMiongozoPage: React.FC = () => {
  ...
  const loadOrodha = async () => {
    try {
      ...
      const response = await publicAPI.getGuidesByCategory('Orodha ya Viwango na Miongozo');
      
      if (response.success && response.data) {
        const orodhaGuides = response.data;
        
        if (orodhaGuides.length > 0) {
          // Set the first featured guide as main document, or first guide if none featured
          const featuredGuide = orodhaGuides.find(g => g.featured) || orodhaGuides[0];
          setMainDocument(featuredGuide);
          
          // Set the rest as related documents
          const related = orodhaGuides.filter(g => g.id !== featuredGuide.id);
          setRelatedDocuments(related);
        } else {
          // No orodha guides found
      ...
      Swal.fire({
        title: 'Makosa',
        text: 'Imeshindwa kupakia orodha. Tafadhali jaribu tena baadaye.',
        icon: 'error',
  ...
  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      // If search is empty, reload all orodha
      loadOrodha();
      return;
    }
    ...
    try {
      ...
      // Fetch orodha guides by category
      const response = await publicAPI.getGuidesByCategory('Orodha ya Viwango na Miongozo');
      ...
      Swal.fire({
        title: 'Makosa ya Utafutaji',
        text: 'Imeshindwa kutafuta orodha. Tafadhali jaribu tena.',
        icon: 'error',
        confirmButtonText: 'Sawa'
      });
    } finally {
  ...
  const handleView = (guide: Guide) => {
    try {
      ...
      const hasFile = guide.file_name && guide.file_path;
      
      if (!hasFile) {
        Swal.fire({
          title: 'File Not Available',
          text: 'Document file not available for viewing',
          icon: 'warning',
          confirmButtonText: 'OK'
        });
        return;
      ...
      Swal.fire({
        title: 'View Failed',
        text: 'Failed to open the document file. Please try again later.',
        icon: 'error',
        confirmButtonText: 'OK'
      });
    }
          ...
          <div className="text-center">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <List className="w-6 h-6 text-blue-600 animate-pulse" />
              </div>
            </div>
            <p className="text-gray-600 text-lg font-medium">Inapakia orodha ya viwango na miongozo...</p>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full mb-6">
              ...
              <span className="text-sm font-medium">Orodha ya Viwango na Miongozo</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-bold mb-6 leading-tight">
              Orodha ya <span className="text-yellow-300">Viwango na Miongozo</span>
            </h1>
            {/* <p className="text-xl md:text-2xl text-blue-100 mb-8 max-w-2xl mx-auto">
              Pata orodha kamili ya viwango na miongozo bora zaidi kwa ajili ya miradi yako ya e-Government
            </p> */}
            <nav className="flex justify-center items-center space-x-2 text-sm text-blue-100">
              ...
              <span>/</span>
              <span className="text-white font-medium">Orodha ya Viwango na Miongozo</span>
            </nav>
          </div>
        </div>
      </section>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Jumla ya Orodha</p>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow">
                ...
                <form onSubmit={handleSearch} className="relative group">
                  ...
                  <div className="relative bg-white rounded-2xl border-2 border-gray-200 focus-within:border-blue-500 transition-colors">
                    <div className="flex items-center">
                      ...
                      <input
                        type="text"
                        placeholder="Tafuta orodha ya viwango na miongozo... (Andika jina, kategoria, au maelezo)"
                        value={searchQuery}
                ...
                <div className="mb-12">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Nyaraka kuu ya orodha ya viwango na miongozo</p>
                    </div>
                  </div>
                    ...
                    <div className="relative">
                      ...
                      <div className="relative bg-white rounded-2xl p-6 border-2 border-blue-200 shadow-lg h-full">
                        ...
                        <p className="text-sm text-gray-700 leading-relaxed mb-4">
                          {mainDocument.description || 'Hakuna maelezo ya nyaraka hii. Nyaraka hii ina orodha kamili ya viwango na miongozo muhimu kwa ajili ya miradi ya e-Government.'}
                        </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {relatedDocuments.length} orodha zilizopatikana
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {relatedDocuments.length > 0 ? (
                  ...
                  <div className="text-center py-16 bg-gradient-to-br from-gray-50 to-blue-50/50 rounded-2xl border-2 border-dashed border-gray-300">
                    ...
                    <p className="text-gray-600 mb-6 max-w-md mx-auto">
                      {searchQuery 
                        ? `Hakuna orodha zinazofanana na "${searchQuery}". Jaribu kutafuta kwa maneno mengine.`
                        : 'Bado hakuna orodha ya viwango na miongozo zimewekwa. Tafadhali rudi baadaye.'}
                    </p>
                    {searchQuery && (
                      <button
                        onClick={() => {
...
export default OrodhaViwangoMiongozoPage;
